...
def main(args):
    # data loading
    data = np.load('data_npz/UMa_tx32_c256_csi_DA_trun.npz')
    ...
    FS_data = np.load('data_npz/UMa_tx32_c256_csi.npz')
    ...
    for i in range(200):
        ...
        scio.savemat('H_UMa_{}.mat'.format(args.encode_dim), mdict={'H': Z_[60, :, :, :,]})
        print("save successfully!!")
        
        x_FS_pre_c = DApowernorm_to_HS(Z, x_test_power_norm_para[i * 100: (i + 1) * 100])
        scio.savemat('H_ed{}.mat'.format(args.encode_dim), mdict={'H_FS_pre_c': x_FS_pre_c[:100, :, :]})
        print("save successfully!!")
        ...
        #print("{} nmse_da {} nmse_sf {} gcs {}".format(i, nmse_da, nmse_sf, gcs))
        ...
        #print("{} nmse_da_pn {} nmse_sf_pn {} gcs_pn {}".format(i, nmse_da_pn, nmse_sf_pn, gcs_pn))
        t2 = time.time()
        #print("one:{}".format(t2 - t1))
        
    np.savez('NMSE_UMa_SVD_POWERNORM_DA_mode{}_{}'.format(mode, encode_dim), NMSE_DA_LIST=NMSE_DA_LIST)
    np.savez('NMSE_UMa_SVD_POWERNORM_SF_mode{}_{}'.format(mode, encode_dim), NMSE_SF_LIST=NMSE_SF_LIST)
    np.savez('GCS_UMa_SVD_POWERNORM_SF_mode{}_{}'.format(mode, encode_dim), GCS_LIST=GCS_LIST)
    np.savez('NMSE_UMa_SVD_POWERNORM_DA_PN_mode{}_{}'.format(mode, encode_dim), NMSE_DA_PN_LIST=NMSE_DA_PN_LIST)
    np.savez('NMSE_UMa_SVD_POWERNORM_SF_PN_mode{}_{}'.format(mode, encode_dim), NMSE_SF_PN_LIST=NMSE_SF_PN_LIST)
    savemat('UMa_H_{}.mat'.format(encode_dim), {'H': H_hat})
...
if __name__ == "__main__":
    ...
    parser.add_argument("-mp", "--model_name_path",
                        default='model_zoo/ffdnet_lr0.0001_b8_lnmse_nnone_w1e-06_d0.0_uma_power_norm.h5', type=str)
